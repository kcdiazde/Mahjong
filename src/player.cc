...
bool Player::FindTileWithIdAndGroup(TileId tile_id,
...
bool Player::IsTileChow(MahjongTile& tile) {
  ...
  bool next_next_tile_found = FindTileWithIdAndGroup(next_next_id, tile_group);
  bool prev_tile_found = FindTileWithIdAndGroup(prev_id, tile_group);
  bool prev_prev_tile_found = FindTileWithIdAndGroup(prev_prev_id, tile_group);

  // Tile is at beggining of chow
  if (next_tile_found && next_next_tile_found) return true;

  // Tile is at middle of chow
  if (prev_tile_found && next_tile_found) return true;

  // Tile is at end of chow
  if (prev_prev_tile_found && prev_tile_found) return true;
...
bool Player::MoveTileBetweenHands(MahjongHand& src_hand, MahjongHand& dst_hand,
  ...
  for (const auto& tile_ptr : src_hand_copy) {
    if (tile_ptr->GetId() == tile_id) {
      ...
      src_hand.erase(std::find(src_hand.begin(), src_hand.end(), tile_ptr));
